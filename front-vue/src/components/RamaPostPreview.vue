      ...
      <header class="flex flex-col">
        <h1 class="text-2xl">
          <router-link
            ...
            :to="{
              name: 'ramaPostShow',
              params: { host: foundationHost, foundationName, postID: post.id },
            }"
          >
            {{ post.title }}
          </router-link>
          ...
          <router-link
            :to="{
              ...
              params: {
                host: foundationHost,
                foundationName,
                authorName: encodeURIComponent(post.author),
              },
            }"
          >
            {{ post.author }}
          </router-link>
          on {{ new Date(post.seal.time).toLocaleString() }}
        </span>
      </header>
    ...
    <div v-if="amJanitor" class="text-right">
      <div class="mt-2 mb-4">
        <span
          ...
          v-if="!toolsExpanded"
          >Show Janitor Tools</span
        >
        <span
          @click="toolsExpanded = false"
          class="text-blue-500 underline cursor-pointer"
          v-if="toolsExpanded"
          >Close Janitor Tools</span
        >
      </div>
      ...
      <div v-if="toolsExpanded" class="flex flex-col justify-end">
        <div class="self-end w-1/3 pb-2 mb-4 border-b">
          ...
          <button class="action-btn" @click="moveToFolder">
            Move to folder
          </button>
        </div>
        ...
        <ul>
          <li v-for="tag in currentTags" :key="tag" class="mb-1">
            ...
            <span
              v-if="amJanitor"
              @click="removeTag(tag)"
              class="p-1 text-sm font-bold text-red-400 underline cursor-pointer dark:text-rose-800"
              >remove</span
            >
          </li>
        </ul>
        <div class="my-2">
          <select v-model="selectedTag" class="p-2 mr-2 rounded-md">
            ...
            <option
              v-for="tagName in potentialTags"
              :key="tagName"
              :value="tagName"
            >
              {{ tagName }}
...
const postLink = computed(() => {
  return `/apps/groups/groups/${props.foundationHost}/${
    props.foundationName
  }/channels/diary/${props.foundationHost}/${
    props.foundationName
  }/note/${removeDots(props.post.id)}`;
});

const theFoundation = computed<R.SubFoundation | null>(() => {
  return store.getters[GetterTypes.HOSTED_FOUNDATION_BY_PROVIDER](
    `${props.foundationHost}/${props.foundationName}`
  );
});
...
const foundationFolders = computed<Array<R.FoldersMeta> | []>(() => {
  ...
  return store.getters[GetterTypes.HOSTED_FOLDERS_BY_PROVIDER](
    `${props.foundationHost}/${props.foundationName}`
  );
});
...
const foundationTags = computed<Array<R.TagsMeta>>(() => {
  ...
  return store.getters[GetterTypes.HOSTED_TAGS_BY_PROVIDER](
    `${props.foundationHost}/${props.foundationName}`
  );
});
