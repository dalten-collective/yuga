    ...
    <header class="justify-center mt-4 mb-12 grid grid-cols-3 gap-4">
      ...
      <div class="text-center">
        <h1 class="text-2xl text-center">
          <div>
            {{ decodedTagName }}
            <span class="text-sm text-gray-400">({{ posts.length }} {{
...
const tag = computed<T.TagsMeta>(() => {
  return store.getters[
    GetterTypes.TAG_BY_DETAILS
  ]({
    host: props.host,
    foundationName: props.foundationName,
    tagName: decodedTagName.value,
  })
})

const decodedTagName = computed(() => {
   return decodeURIComponent(props.tagName)
})
...
const getPosts = (): void => {
  ...
  const postIDs = tag.value.posts.map((post: T.FolderPost) => post.id)
  Promise.all(
