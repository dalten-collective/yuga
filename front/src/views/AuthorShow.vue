...
const author = computed<T.AuthorsMeta>(() => {
  return store.getters[
    GetterTypes.AUTHOR_BY_DETAILS
  ]({
    host: props.host,
    foundationName: props.foundationName,
    authorName: decodedAuthorName.value,
  })
})
...
const getPosts = (): void => {
  ...
  const postIDs = author.value.posts.map((post: T.FolderPost) => post.id)
  Promise.all(
